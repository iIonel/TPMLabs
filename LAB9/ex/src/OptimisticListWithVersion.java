...
public class OptimisticListWithVersion<T> {
    private Node head;

    public OptimisticListWithVersion() {
    ...
    private boolean validate(Node pred, Node current) {
        ...
        while (entry.key <= pred.key) {
            if (entry == pred)
                return pred.next == current && entry.version == pred.version && entry.version == current.version;
            entry = entry.next;
        }
        return false;
    }
